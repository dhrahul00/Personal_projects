...
class ConV2D(Layer):
    ...
    def zero_pad(self,X,pad):
        return (np.pad(X,((0,0),(pad,pad),(pad,pad),(0,0)),mode="constant",constant_values = 0)).astype(float)
    ...
    def forward_prop(self,input_):
        ...
        self.weights_ = np.random.randn(self.filters,self.kernel_size[0],self.kernel_size[1],self.n_c_img)/np.sqrt(self.kernel_size[0])
        self.bias_ = np.random.randn(self.filters,1,1,1)
        
        
        n_c,f,f,n_c_prev = self.weights_.shape
        
        n_h = (self.n_h_img-f+2*self.pad)//self.stride + 1
        n_w = (self.n_w_img-f+2*self.pad)//self.stride + 1
