...
from [New_db].[dbo].[sales_data_sample]
group by  DEALSIZE
order by 2 desc
...
select CUSTOMERNAME , rfm_recency, rfm_frequency, rfm_monetary,
	case  
		when rfm_cell_string in (111, 112 , 121, 122, 123, 144, 132, 211, 212, 114, 141) then 'lost_customers'  --lost custome rs
...
select distinct OrderNumber, stuff(
	...
	where ORDERNUMBER in 
		(
			...
			where rn = 3 -- This can be changed to check how many oredr sold together
		)
